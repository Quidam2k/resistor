...
1. **Response capture + tracking loop** — **DONE 2026-08-18.** `db.record_response()` logs a reply to the `responses` table AND writes a markdown copy to `data/responses/`; `tracker.response_search_queries()` gives Claude the Gmail sender filters to run at session start (docstring warns to filter out the reps' newsletter blasts). Surfaced on the `python -m src.tracker` scoreboard. Threads into future letters via the existing `get_prior_correspondence()` LEFT JOIN.
